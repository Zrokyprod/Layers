...
def resolve_model_context_limit(model: str | None) -> ModelContextLimitResolution:
    """Return the best model context limit plus source/confidence metadata."""
...
def known_model_context_limit(model: str | None) -> int | None:
    """Return a context limit only when the model is known."""
...
def is_token_overflow_error_message(error_message: Any) -> bool:
    return match_token_overflow_error_pattern(error_message) is not None
