...
const api = vi.hoisted(() => ({
  ...
  reconcileSavedPostgresRead: vi.fn(),
...
const readyPostgresReadiness = {
  ...
  blockers: [],
  last_checked_at: "2026-06-20T09:20:00Z",
};

describe("SystemOfRecordConnectors", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.clearAllMocks();
    ...
    api.getPostgresReadConnectorStatus.mockResolvedValue({
      connected: true,
      connector_type: "postgres_read",
      ...
      has_read_query: true,
      read_query_digest: "sha256:postgres-query",
      has_bearer_token: false,
      bearer_token_last4: null,
      last_tested_at: "2026-06-20T09:20:00Z",
      health_status: "healthy",
      last_verdict: "matched",
      last_error: null,
      last_error_code: null,
      ...
      last_retryable: false,
      last_checked_at: "2026-06-20T09:20:00Z",
      readiness: readyPostgresReadiness,
      created_at: "2026-06-20T08:00:00Z",
      updated_at: "2026-06-20T08:30:00Z",
    });
    ...
    api.savePostgresReadConnectorConfig.mockResolvedValue({
      connected: true,
      connector_type: "postgres_read",
      base_url: "postgresql://db.example.com/app",
      path_template: null,
    ...
    api.testPostgresReadConnector.mockResolvedValue({
      ...
      connector: {
        ...
        readiness: readyPostgresReadiness,
        created_at: "2026-06-20T08:00:00Z",
        updated_at: "2026-06-20T09:20:00Z",
      },
      check: {
        id: "check_postgres_test",
        project_id: "proj_1",
        call_id: null,
        trace_id: null,
        runtime_policy_decision_id: null,
        ...
        amount_usd: null,
        currency: null,
        claimed: { ticket_id: "TIC-1001", status: "closed" },
        actual: { ticket_id: "TIC-1001", status: "closed" },
        comparison: { compared_fields: [], mismatches: [] },
        idempotency_key: null,
    ...
    api.reconcileSavedPostgresRead.mockResolvedValue({
      id: "check_postgres_saved",
      project_id: "proj_1",
      call_id: null,
      trace_id: null,
      runtime_policy_decision_id: null,
      ...
      comparison: { compared_fields: [], mismatches: [] },
      idempotency_key: null,
      metadata: { connector_kind: "postgres_read", source: "dashboard_saved_connector_proof" },
      checked_at: "2026-06-20T09:25:00Z",
      created_at: "2026-06-20T09:25:00Z",
    });
  ...
  it("shows GitHub beside alert delivery integrations", async () => {
    render(<SystemOfRecordConnectors />);
    ...
    expect(api.getPostgresReadConnectorStatus).toHaveBeenCalledTimes(1);
  ...
  it("surfaces ledger preflight handoff command and downloadable template", async () => {
    ...
    expect(fullProofCommand.textContent).toContain("--scenario refund --api-base-url https://api.zroky.com");
  ...
  it("surfaces customer preflight handoff command and downloadable template", async () => {
    ...
    expect(fullProofCommand.textContent).toContain("--scenario customer-record --api-base-url https://api.zroky.com");
  ...
  it("configures and runs PostgreSQL read proof without leaking the database URL", async () => {
    render(<SystemOfRecordConnectors />);

    const heading = await screen.findByRole("heading", { name: "PostgreSQL read connector" });
    const connectorCard = heading.closest("article");
    expect(connectorCard).not.toBeNull();

    expect(within(connectorCard as HTMLElement).getByLabelText("PostgreSQL read connector status")).toBeInTheDocument();
    expect(within(connectorCard as HTMLElement).getByText("postgresql://db.***/...")).toBeInTheDocument();
    expect(within(connectorCard as HTMLElement).getByText("Stored DSN ending /app")).toBeInTheDocument();

    fireEvent.change(within(connectorCard as HTMLElement).getByLabelText("Database URL"), {
      target: { value: "postgresql://readonly:pg-secret@db.example.com/app" },
    });
    fireEvent.change(within(connectorCard as HTMLElement).getByLabelText("Read-only query"), {
      target: { value: "SELECT ticket_id, status FROM tickets WHERE ticket_id = :ticket_id" },
    });
    fireEvent.click(within(connectorCard as HTMLElement).getByRole("button", { name: "Save PostgreSQL connector" }));
    ...
    expect(await screen.findByText("PostgreSQL read connector saved.")).toBeInTheDocument();
    expect(document.body.textContent).not.toContain("pg-secret");

    fireEvent.change(within(connectorCard as HTMLElement).getByLabelText("Query params JSON"), {
      target: { value: '{ "ticket_id": "TIC-1001" }' },
    });
    fireEvent.change(within(connectorCard as HTMLElement).getByLabelText("Claimed record JSON"), {
      target: { value: '{ "ticket_id": "TIC-1001", "status": "closed" }' },
    });
    fireEvent.click(within(connectorCard as HTMLElement).getByRole("button", { name: "Run PostgreSQL test reconciliation" }));

    await waitFor(() =>
      expect(api.testPostgresReadConnector).toHaveBeenCalledWith({
        claimed: { ticket_id: "TIC-1001", status: "closed" },
        params: { ticket_id: "TIC-1001" },
        system_ref: "postgres:tickets:TIC-1001",
        action_type: "postgres_read_verification",
        match_fields: ["ticket_id", "status"],
      }),
    );
    expect(await screen.findByText("PostgreSQL read test recorded matched.")).toBeInTheDocument();

    fireEvent.click(within(connectorCard as HTMLElement).getByRole("button", { name: "Run saved PostgreSQL proof" }));

    await waitFor(() =>
      expect(api.reconcileSavedPostgresRead).toHaveBeenCalledWith({
        claimed: { ticket_id: "TIC-1001", status: "closed" },
        params: { ticket_id: "TIC-1001" },
        system_ref: "postgres:tickets:TIC-1001",
        action_type: "postgres_read_verification",
    ...
    expect(JSON.stringify(api.reconcileSavedPostgresRead.mock.calls[0]?.[0])).not.toContain("postgresql://");
    expect(await screen.findByText("PostgreSQL saved proof recorded matched.")).toBeInTheDocument();
    expect(screen.getAllByText("postgres:tickets:TIC-1001").length).toBeGreaterThan(0);
  });
});
