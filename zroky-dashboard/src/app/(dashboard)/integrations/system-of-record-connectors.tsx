...
type IntegrationState = {
  ...
  postgresConfig: PostgresReadConnectorStatusResponse | null;
  outcomeChecks: OutcomeReconciliationView[];
};

type ConnectorEvidenceKind = "ledger" | "customer" | "postgres";
...
type PostgresConnectorForm = {
  databaseUrl: string;
  readQuery: string;
  testParamsJson: string;
  testClaimedJson: string;
  testSystemRef: string;
};

const defaultPostgresForm: PostgresConnectorForm = {
  databaseUrl: "",
  readQuery: "SELECT ticket_id, status FROM tickets WHERE ticket_id = :ticket_id",
  testParamsJson: '{\n  "ticket_id": "TIC-1001"\n}',
  testClaimedJson: '{\n  "ticket_id": "TIC-1001",\n  "status": "closed"\n}',
  testSystemRef: "postgres:tickets:TIC-1001",
};
...
const defaultConnectorContracts: Record<ConnectorEvidenceKind, SystemOfRecordConnectorContract> = {
  ...
  postgres: {
    ...
    system_of_record: "postgres_read_model",
    required_inputs: [
      "database_url_secret",
      "read_only_select_query",
      "safe_test_params",
    ],
    required_record_fields: ["record_id", "status"],
    recommended_record_fields: ["updated_at", "owner_id"],
  },
};
...
function connectorReadiness({
  ...
  const checks: Record<string, boolean> =
    kind === "postgres"
      ? {
          ...
          database_url_present: hasDatabaseUrl === true,
          read_query_present: hasReadQuery === true,
  ...
  const blockerMessages: Record<string, string> = {
    ...
    read_query_present: "Read-only PostgreSQL query is missing.",
...
function isPostgresReadCheck(item: OutcomeReconciliationView) {
  const metadata = isRecord(item.metadata) ? item.metadata : {};
  return item.connector_type === "postgres_read" || metadata.connector_kind === "postgres_read";
}
...
function connectorFixGuidance({
  ...
  const surface = kind === "ledger" ? "ledger/refund" : kind === "customer" ? "CRM/customer" : "PostgreSQL read";
  ...
  if (code === "auth_failed" || status === "401" || status === "403") {
    if (kind === "postgres") {
      return "Fix PostgreSQL auth: rotate the read-only database URL, confirm network access, then rerun preflight.";
    }
    return `Fix ${surface} auth: rotate the bearer token, confirm scopes, then rerun preflight.`;
  }
  if (code === "connector_timeout") {
    if (kind === "postgres") {
      return "Fix PostgreSQL reachability: confirm the runner can reach the database and the query returns within timeout.";
    }
  ...
  if (code === "connector_config_invalid") {
    if (kind === "postgres") {
      return "Fix PostgreSQL config: use a read-only SELECT query with named params and a scoped database user.";
    }
...
function connectorEvidenceLabel(kind: ConnectorEvidenceKind) {
  ...
  if (kind === "customer") return "Customer record";
  return "PostgreSQL read";
}
...
function connectorPreflightLabel(kind: ConnectorEvidenceKind) {
  if (kind === "ledger") return "Ledger refund";
  if (kind === "customer") return "Customer record";
  return "PostgreSQL read";
}

function connectorPreflightKind(kind: ConnectorEvidenceKind) {
  if (kind === "ledger") return "ledger_refund_api";
  if (kind === "customer") return "customer_record_api";
...
function ledgerPreflightCommand(form: LedgerConnectorForm) {
  return `python scripts/run_design_partner_install_kit.py --scenario refund --preflight-only --api-base-url https://api.zroky.com --api-key <zroky_api_key> --ledger-base-url ${commandValue(form.baseUrl, "https://ledger.example.com/api")} --ledger-bearer-token <ledger_token> --refund-id ${commandValue(form.testRefundId, "<refund_id>")} --json --write-summary artifacts/design-partner-refund-preflight-summary.json`;
}

function customerPreflightCommand(form: CustomerConnectorForm) {
  return `python scripts/run_design_partner_install_kit.py --scenario customer-record --preflight-only --api-base-url https://api.zroky.com --api-key <zroky_api_key> --crm-base-url ${commandValue(form.baseUrl, "https://crm.example.com/api")} --crm-bearer-token <crm_token> --customer-id ${commandValue(form.testCustomerId, "<customer_id>")} --json --write-summary artifacts/design-partner-crm-preflight-summary.json`;
}

function ledgerFullProofCommand(form: LedgerConnectorForm) {
  return `python scripts/run_design_partner_install_kit.py --scenario refund --api-base-url https://api.zroky.com --api-key <zroky_api_key> --ledger-base-url ${commandValue(form.baseUrl, "https://ledger.example.com/api")} --ledger-bearer-token <ledger_token> --refund-id ${commandValue(form.testRefundId, "<refund_id>")} --json --write-summary artifacts/design-partner-refund-live-summary.json --write-evidence artifacts/design-partner-refund-live-evidence.json`;
}

function customerFullProofCommand(form: CustomerConnectorForm) {
  return `python scripts/run_design_partner_install_kit.py --scenario customer-record --api-base-url https://api.zroky.com --api-key <zroky_api_key> --crm-base-url ${commandValue(form.baseUrl, "https://crm.example.com/api")} --crm-bearer-token <crm_token> --customer-id ${commandValue(form.testCustomerId, "<customer_id>")} --json --write-summary artifacts/design-partner-crm-live-summary.json --write-evidence artifacts/design-partner-crm-live-evidence.json`;
}

function postgresPreflightCommand(form: PostgresConnectorForm) {
  const systemRef = commandValue(form.testSystemRef, "postgres:tickets:TIC-1001");
  return `curl -X POST "$ZROKY_API_BASE/v1/integrations/system-of-record/postgres-read/test" -H "x-api-key: $ZROKY_API_KEY" -H "Content-Type: application/json" -d '{"system_ref":"${systemRef}","params":${compactJsonForCommand(form.testParamsJson)},"claimed":${compactJsonForCommand(form.testClaimedJson)},"match_fields":["ticket_id","status"]}'`;
}

function postgresFullProofCommand(form: PostgresConnectorForm) {
  const systemRef = commandValue(form.testSystemRef, "postgres:tickets:TIC-1001");
  return `curl -X POST "$ZROKY_API_BASE/v1/outcomes/reconciliation/postgres-read/saved" -H "x-api-key: $ZROKY_API_KEY" -H "Content-Type: application/json" -d '{"system_ref":"${systemRef}","params":${compactJsonForCommand(form.testParamsJson)},"claimed":${compactJsonForCommand(form.testClaimedJson)},"match_fields":["ticket_id","status"],"metadata":{"source":"dashboard_saved_connector_proof"}}'`;
}
...
function postgresConnectorTemplate(form: PostgresConnectorForm) {
  return {
    connector_type: "postgres_read",
    ...
    saved_proof_endpoint: "/v1/outcomes/reconciliation/postgres-read/saved",
    config_payload: {
      database_url: "<postgres_read_only_database_url>",
      read_query: commandValue(
        form.readQuery,
        "SELECT ticket_id, status FROM tickets WHERE ticket_id = :ticket_id",
      ),
    },
    test_payload: {
      system_ref: commandValue(form.testSystemRef, "postgres:tickets:TIC-1001"),
      params: parseRecordForTemplate(form.testParamsJson, { ticket_id: "TIC-1001" }),
      claimed: parseRecordForTemplate(form.testClaimedJson, {
        ticket_id: "TIC-1001",
        status: "closed",
      }),
      match_fields: ["ticket_id", "status"],
      metadata: {
        install_kit: "design_partner_postgres_read_v1",
        partner_run_id: "<partner_run_id>",
      },
    },
    readiness_contract: {
      ...defaultConnectorContracts.postgres,
      ready_when: [
        "config_saved",
...
function postgresReadPayloadSnippet() {
  return `curl -X POST "$ZROKY_API_BASE/v1/integrations/system-of-record/postgres-read/test" \\
  -H "x-api-key: $ZROKY_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
    "system_ref": "postgres:tickets:TIC-1001",
    "params": {
      "ticket_id": "TIC-1001"
    },
    "claimed": {
...
function formFromPostgresConfig(
  config: PostgresReadConnectorStatusResponse | null,
  previous: PostgresConnectorForm,
...
function compactJsonForCommand(value: string) {
  const parsed = parseRecordForTemplate(value, {});
  return JSON.stringify(parsed);
}

function parseRecordForTemplate(value: string, fallback: Record<string, unknown>) {
  try {
...
function parsePostgresParams(value: string) {
  const parsed = parseJsonRecord(value, "PostgreSQL params");
  ...
  for (const [key, paramValue] of Object.entries(parsed)) {
    ...
    throw new Error("PostgreSQL params can only contain string, number, boolean, or null values.");
  }
  return params;
}

export default function SystemOfRecordConnectors() {
  ...
  const [postgresForm, setPostgresForm] = useState<PostgresConnectorForm>(defaultPostgresForm);
  ...
  const [runningPostgresSavedProof, setRunningPostgresSavedProof] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    const [
      ...
      slackResult,
      ledgerConfigResult,
      customerConfigResult,
      postgresConfigResult,
      outcomeResult,
    ...
    const postgresConfig = postgresConfigResult.status === "fulfilled" ? postgresConfigResult.value : null;
    ...
    setPostgresForm((prev) => formFromPostgresConfig(postgresConfig, prev));
  ...
  const postgresConnected = Boolean(state.postgresConfig?.connected);
  ...
  const postgresReadChecks = state.outcomeChecks.filter(isPostgresReadCheck);
  const latestLedgerRefundCheck = ledgerRefundChecks[0] ?? null;
  const latestCustomerRecordCheck = customerRecordChecks[0] ?? null;
  const latestPostgresReadCheck = postgresReadChecks[0] ?? null;
  ...
  const postgresMetadata = connectorMetadata(latestPostgresReadCheck);
  ...
  const postgresHealth = state.postgresConfig?.health_status ?? (postgresConnected ? "not_verified" : "not_configured");
  ...
  const postgresLastVerdict = state.postgresConfig?.last_verdict ?? latestPostgresReadCheck?.verdict;
  ...
  const postgresLastRetryable = state.postgresConfig?.last_retryable ?? boolValue(postgresMetadata.retryable);
  ...
  const postgresErrorCode = state.postgresConfig?.last_error_code ?? textValue(postgresMetadata.error_code);
  ...
  const postgresRawError = state.postgresConfig?.last_error ?? textValue(postgresMetadata.error);
  ...
  const postgresHttpStatusValue = state.postgresConfig?.last_http_status ?? postgresMetadata.http_status;
  ...
  const postgresAttemptValue = state.postgresConfig?.last_attempts ?? postgresMetadata.attempts;
  ...
  const postgresReadiness = connectorReadiness({
    kind: "postgres",
    connected: postgresConnected,
    hasBearerToken: false,
    hasDatabaseUrl: state.postgresConfig?.has_database_url === true,
    hasReadQuery: state.postgresConfig?.has_read_query === true,
    verdict: postgresLastVerdict,
    errorCode: postgresErrorCode,
    httpStatus: postgresHttpStatusValue,
    attempts: postgresAttemptValue,
    retryable: postgresLastRetryable,
    readiness: state.postgresConfig?.readiness,
  });
  const ledgerReadinessStatus = ledgerReadiness.status;
  const customerReadinessStatus = customerReadiness.status;
  const postgresReadinessStatus = postgresReadiness.status;
  const ledgerReadinessBlockers = ledgerReadiness.blockers ?? [];
  const customerReadinessBlockers = customerReadiness.blockers ?? [];
  const postgresReadinessBlockers = postgresReadiness.blockers ?? [];
  ...
  const postgresContract = postgresReadiness.contract ?? defaultConnectorContracts.postgres;
  const readyCount = [
    githubConnected,
    slackConnected,
    ledgerConnected && ledgerReadinessStatus === "ready",
    customerConnected && customerReadinessStatus === "ready",
    postgresConnected && postgresReadinessStatus === "ready",
  ...
  const postgresLastError = connectorErrorLabel(
    postgresErrorCode,
    postgresRawError,
    postgresLastRetryable,
  );
  ...
  const postgresGuidance = connectorFixGuidance({
    kind: "postgres",
    connected: postgresConnected,
  ...
  const postgresReadyForPilot = connectorReadyForPilot(
    postgresConnected,
    postgresHealth,
    postgresLastVerdict,
    postgresErrorCode,
  ...
  const postgresFailedAttempts = failedPreflightAttempts(postgresReadChecks);
  ...
  const postgresRequestUrl = maskedConnectorUrl(state.postgresConfig?.base_url ?? postgresMetadata.request_url);
  const postgresQueryDigest = state.postgresConfig?.read_query_digest ?? textValue(postgresMetadata.query_digest);
  ...
  const customerTokenStatus = state.customerConfig?.has_bearer_token
    ...
    : "No bearer token stored";
  const postgresDatabaseUrlStatus = state.postgresConfig?.has_database_url
    ? `Stored DSN ending ${state.postgresConfig.database_url_last4 ?? "****"}`
  ...
  async function copyPostgresPayload() {
    try {
      await navigator.clipboard.writeText(postgresReadPayloadSnippet());
      setMessage("PostgreSQL read saved-connector test payload copied.");
    } catch {
      setMessage("Copy failed. Select the payload and copy it manually.");
    }
  }
  ...
  async function copyPostgresPreflightCommand() {
    try {
      await navigator.clipboard.writeText(postgresPreflight);
      setMessage("PostgreSQL read preflight command copied.");
    } catch {
      setMessage("Copy failed. Select the preflight command and copy it manually.");
    }
  }
  ...
  async function copyPostgresFullProofCommand() {
    try {
      await navigator.clipboard.writeText(postgresFullProof);
      setMessage("PostgreSQL read full proof command copied.");
    } catch {
      setMessage("Copy failed. Select the full proof command and copy it manually.");
    }
  }
  ...
  function downloadPostgresTemplate() {
    downloadJsonFile(postgresConnectorTemplate(postgresForm), "postgres_read_connector_config.example.json");
    setMessage("PostgreSQL read connector template downloaded.");
  }
  ...
  function downloadPostgresPreflightSummary() {
    downloadJsonFile(
      buildConnectorPreflightSummary({
        kind: "postgres",
        connected: postgresConnected,
        ...
        verdict: postgresLastVerdict,
        errorCode: postgresErrorCode,
        error: postgresRawError,
        httpStatus: postgresHttpStatusValue,
        attempts: postgresAttemptValue,
        retryable: postgresLastRetryable,
        readiness: postgresReadiness,
        guidance: postgresGuidance,
        readyForPilot: postgresReadyForPilot,
        fullProofCommand: postgresFullProof,
        latestCheck: latestPostgresReadCheck,
        failedAttempts: postgresFailedAttempts,
      }),
      "postgres_read_preflight_summary.json",
    );
    setMessage("PostgreSQL read preflight summary downloaded.");
  }
  ...
  function updatePostgresForm(field: keyof PostgresConnectorForm, value: string) {
  ...
  async function onSavePostgresConnector(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setMessage("");
    setSavingPostgres(true);
    try {
      const updated = await savePostgresReadConnectorConfig({
        database_url: optionalText(postgresForm.databaseUrl),
        read_query: postgresForm.readQuery.trim(),
      });
      setState((prev) => ({ ...prev, postgresConfig: updated }));
      setPostgresForm((prev) => formFromPostgresConfig(updated, { ...prev, databaseUrl: "" }));
      setMessage("PostgreSQL read connector saved.");
    } catch (saveError) {
      setMessage(saveError instanceof Error ? saveError.message : "Failed to save PostgreSQL read connector.");
    } finally {
  ...
  async function onTestPostgresConnector(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setMessage("");
    setTestingPostgres(true);

    try {
      const params = parsePostgresParams(postgresForm.testParamsJson);
      const claimed = parseJsonRecord(postgresForm.testClaimedJson, "PostgreSQL claimed record");
      const result = await testPostgresReadConnector({
        claimed,
        params,
        system_ref: optionalText(postgresForm.testSystemRef),
        action_type: "postgres_read_verification",
        match_fields: Object.keys(claimed),
      });
      setState((prev) => ({
        ...prev,
        postgresConfig: result.connector,
        outcomeChecks: [
          result.check,
          ...prev.outcomeChecks.filter((item) => item.id !== result.check.id),
        ].slice(0, 25),
      }));
      setMessage(`PostgreSQL read test recorded ${result.check.verdict}.`);
    } catch (testError) {
      setMessage(testError instanceof Error ? testError.message : "Failed to run PostgreSQL read test.");
    } finally {
  ...
  async function onRunSavedPostgresProof() {
    setMessage("");
    setRunningPostgresSavedProof(true);

    try {
      const params = parsePostgresParams(postgresForm.testParamsJson);
      const claimed = parseJsonRecord(postgresForm.testClaimedJson, "PostgreSQL claimed record");
      const check = await reconcileSavedPostgresRead({
        claimed,
        params,
        system_ref: optionalText(postgresForm.testSystemRef),
        action_type: "postgres_read_verification",
      ...
      setMessage(`PostgreSQL saved proof recorded ${check.verdict}.`);
    } catch (proofError) {
      setMessage(proofError instanceof Error ? proofError.message : "Failed to run saved PostgreSQL proof.");
    } finally {
      setRunningPostgresSavedProof(false);
    }
  }
    ...
    <div className="page-content settings-integrations-page">
      ...
      <section className="settings-summary-grid">
        ...
        <article className="panel settings-summary-card">
          ...
          <strong>{ledgerConnected || customerConnected || postgresConnected ? `${Number(ledgerReadinessStatus === "ready") + Number(customerReadinessStatus === "ready") + Number(postgresReadinessStatus === "ready")}/3 ready` : "Not configured"}</strong>
          <small>{ledgerConnected || customerConnected || postgresConnected ? `Ledger ${connectorReadinessLabel(ledgerReadinessStatus)} / CRM ${connectorReadinessLabel(customerReadinessStatus)} / Postgres ${connectorReadinessLabel(postgresReadinessStatus)}` : "Save a connector before running proof."}</small>
        </article>
      </section>
      ...
      <section className="settings-integration-grid">
        ...
        <article className="panel settings-integration-card">
          ...
          <div className="actions">
            <Link href="/integrations/slack" className="btn btn-primary">
              Manage Slack
            </Link>
          </div>
        </article>
        ...
        <article className="panel settings-integration-card settings-connector-card" id="postgres-read-connector">
          <header className="panel-header">
            <div className="settings-card-title-row">
              <DatabaseZap aria-hidden="true" />
              <div>
                <h3>PostgreSQL read connector</h3>
                <p>Read-only source-of-record verification for internal tables, ledgers, and custom workflow state.</p>
              </div>
            </div>
            <span className={postgresConnected ? connectorReadinessPillClass(postgresReadinessStatus) : "pill"}>
              {postgresConnected ? connectorReadinessLabel(postgresReadinessStatus) : "Not configured"}
            </span>
          </header>

          <div className="settings-connector-facts" aria-label="PostgreSQL read connector status">
            <div>
              <span>Masked database</span>
              <strong>{postgresRequestUrl}</strong>
            </div>
            <div>
              <span>Health</span>
              <strong>{connectorHealthLabel(postgresHealth)}</strong>
            </div>
            <div>
              <span>Readiness</span>
              <strong>{connectorReadinessLabel(postgresReadinessStatus)}</strong>
            </div>
            <div>
              <span>Last verdict</span>
              <strong>{connectorVerdictLabel(postgresLastVerdict)}</strong>
            </div>
            <div>
              <span>Last error</span>
              <strong>{postgresLastError}</strong>
            </div>
            <div>
              <span>Attempts</span>
              <strong>{textValue(postgresAttemptValue) ?? "No run yet"}</strong>
            </div>
            <div>
              <span>Read query</span>
              <strong>{postgresQueryDigest ? `Digest ${postgresQueryDigest}` : "No query digest yet"}</strong>
            </div>
            <div>
              <span>Database URL</span>
              <strong>{postgresDatabaseUrlStatus}</strong>
            </div>
          </div>

          <section className="settings-connector-guidance" aria-label="PostgreSQL read preflight guidance">
            <div>
              <span className="eyebrow">Preflight fix</span>
              <strong>{postgresReadinessStatus === "ready" ? "Pilot ready" : "Action required"}</strong>
            </div>
            <p>{postgresGuidance}</p>
          </section>

          <div className="settings-connector-flow" aria-label="PostgreSQL read proof flow">
            <div className="settings-connector-flow-item">
              <span className="settings-connector-flow-index">01</span>
              <span>Config</span>
              <strong>{postgresConnected ? "Saved" : "Not configured"}</strong>
            </div>
            <div className="settings-connector-flow-item">
              <span className="settings-connector-flow-index">02</span>
              <span>Preflight</span>
            ...
            <div className="settings-connector-flow-item">
              <span className="settings-connector-flow-index">03</span>
              <span>Saved proof</span>
              <strong>{connectorStatus(latestPostgresReadCheck)}</strong>
            </div>
            <div className="settings-connector-flow-item">
              <span className="settings-connector-flow-index">04</span>
              <span>Evidence</span>
              <strong>{evidenceReadinessLabel(latestPostgresReadCheck)}</strong>
            </div>
          </div>

          <form className="settings-connector-form" onSubmit={(event) => void onSavePostgresConnector(event)}>
            <div className="settings-connector-stage-heading">
              <span>01</span>
              <div>
                <strong>Connector setup</strong>
                <p>Store the read-only database URL as an encrypted secret and bind it to one SELECT query.</p>
              </div>
            </div>
            <div className="settings-connector-form-grid">
              <label>
                ...
                <input
                  type="password"
                  value={postgresForm.databaseUrl}
                  onChange={(event) => updatePostgresForm("databaseUrl", event.target.value)}
                  placeholder={state.postgresConfig?.has_database_url ? "Paste to rotate stored DSN" : "postgresql://readonly:secret@db.example.com/app"}
                />
              </label>
              <label>
                ...
                <textarea
                  rows={4}
                  value={postgresForm.readQuery}
                  onChange={(event) => updatePostgresForm("readQuery", event.target.value)}
                  placeholder="SELECT ticket_id, status FROM tickets WHERE ticket_id = :ticket_id"
                  required
                />
              </label>
            </div>
            <div className="actions">
              <button
                ...
                disabled={
                  savingPostgres ||
                  !postgresForm.readQuery.trim() ||
                  (!postgresForm.databaseUrl.trim() && !state.postgresConfig?.has_database_url)
                }
              >
                <Save aria-hidden="true" />
                {savingPostgres ? "Saving..." : "Save PostgreSQL connector"}
              </button>
            </div>
          </form>

          <form className="settings-connector-test" onSubmit={(event) => void onTestPostgresConnector(event)}>
            <div className="settings-connector-stage-heading">
              <span>02</span>
              <div>
                <strong>Runtime proof</strong>
                <p>Run a saved read against source-of-record state and compare exact claimed fields.</p>
              </div>
            </div>
            <div className="settings-connector-test-grid">
              <label>
                ...
                <input
                  value={postgresForm.testSystemRef}
                  onChange={(event) => updatePostgresForm("testSystemRef", event.target.value)}
                  placeholder="postgres:tickets:TIC-1001"
                />
              </label>
              <label>
                ...
                <textarea
                  rows={5}
                  value={postgresForm.testParamsJson}
                  onChange={(event) => updatePostgresForm("testParamsJson", event.target.value)}
                  placeholder='{"ticket_id":"TIC-1001"}'
                  required
                />
              </label>
              <label>
                <span>Claimed record JSON</span>
                <textarea
                  rows={5}
                  value={postgresForm.testClaimedJson}
                  onChange={(event) => updatePostgresForm("testClaimedJson", event.target.value)}
                  placeholder='{"ticket_id":"TIC-1001","status":"closed"}'
                  required
                />
              </label>
            </div>
            <div className="actions">
              <button
                ...
                disabled={!postgresConnected || testingPostgres || !postgresForm.testParamsJson.trim() || !postgresForm.testClaimedJson.trim()}
              >
                <PlayCircle aria-hidden="true" />
                {testingPostgres ? "Running..." : "Run PostgreSQL test reconciliation"}
              </button>
              <button
                type="button"
                className="btn btn-primary"
                disabled={
                  !postgresConnected ||
                  testingPostgres ||
                  runningPostgresSavedProof ||
                  !postgresForm.testParamsJson.trim() ||
                  !postgresForm.testClaimedJson.trim()
                }
                onClick={() => void onRunSavedPostgresProof()}
              >
                <PlayCircle aria-hidden="true" />
                {runningPostgresSavedProof ? "Running..." : "Run saved PostgreSQL proof"}
              </button>
            </div>
          </form>

          <section className="settings-connector-handoff settings-connector-stage" aria-label="PostgreSQL read pilot handoff">
            <div className="settings-connector-handoff-header">
              <FileJson aria-hidden="true" />
              <div>
                <h4>Pilot proof handoff</h4>
                <p>demos/design-partner-install-kit/postgres_read_connector_config.example.json</p>
              </div>
            </div>
            <p className="settings-connector-muted">Use a scoped read-only role. Never give agent-held credentials for protected production actions.</p>
            <span className="eyebrow">Preflight command</span>
            <pre className="settings-connector-payload" aria-label="PostgreSQL read preflight command">
              <code>{postgresPreflight}</code>
            </pre>
            <span className="eyebrow">Full proof command</span>
            <pre className="settings-connector-payload" aria-label="PostgreSQL read full proof command">
              <code>{postgresFullProof}</code>
            </pre>
            <div className="actions">
              <button type="button" className="btn btn-soft" onClick={() => void copyPostgresPreflightCommand()}>
                <Copy aria-hidden="true" />
                Copy preflight command
              </button>
              <button type="button" className="btn btn-soft" onClick={() => void copyPostgresFullProofCommand()}>
                <Copy aria-hidden="true" />
                Copy full proof command
              </button>
              <button type="button" className="btn btn-soft" onClick={downloadPostgresTemplate}>
                <Download aria-hidden="true" />
                Download template JSON
              </button>
            </div>
          ...
          {renderPreflightSummary(
            "postgres",
            postgresReadyForPilot,
            latestPostgresReadCheck,
            postgresFailedAttempts,
            downloadPostgresPreflightSummary,
          )}
          {renderReadinessContract("postgres", postgresReadiness, postgresContract)}
          {renderFailedPreflightAttempts("postgres", postgresFailedAttempts)}

          <section className="settings-connector-proof-panel" aria-label="PostgreSQL read evidence export">
            <div className="settings-connector-stage-heading">
              <span>04</span>
              <div>
                <strong>Evidence export</strong>
            ...
            <div className="list settings-connector-proof">
              <div className="list-row">
                <div className="list-main">
                  <strong>System reference</strong>
                  <span>{latestPostgresReadCheck?.system_ref ?? "Run the first reconciliation to link a PostgreSQL record."}</span>
                </div>
              </div>
              <div className="list-row">
                <div className="list-main">
                  <strong>Last verdict</strong>
                  <span>{latestPostgresReadCheck?.verdict ?? "not_verified"}</span>
                </div>
              </div>
              {renderEvidenceControl(latestPostgresReadCheck, "postgres")}
            </div>
            {renderEvidenceSummary(latestPostgresReadCheck, "postgres")}
          </section>

          <pre className="settings-connector-payload" aria-label="PostgreSQL read saved connector test payload">
            <code>{postgresReadPayloadSnippet()}</code>
          </pre>

          <div className="actions">
            <button type="button" className="btn btn-soft" onClick={() => void copyPostgresPayload()}>
              <Copy aria-hidden="true" />
