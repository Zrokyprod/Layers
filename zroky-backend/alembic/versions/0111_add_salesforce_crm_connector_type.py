"""add salesforce crm connector type

Revision ID: 0111_add_salesforce_crm_connector_type
Revises: 0110_add_zendesk_ticket_connector_type
...
revision = "0111_add_salesforce_crm_connector_type"
down_revision = "0110_add_zendesk_ticket_connector_type"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    op.execute(
        ...
        "WHERE connector_type = 'salesforce_crm'"
    )
    with op.batch_alter_table(_TABLE) as batch_op:
        batch_op.drop_constraint(_CHECK_NAME, type_="check")
