...
_ISO_TS_RE = re.compile(r"\b\d{4}-\d{2}-\d{2}[tT ]\d{2}:\d{2}:\d{2}(?:\.\d+)?(?:Z|[+-]\d{2}:?\d{2})?\b")
...
def is_similar_output(left: Any, right: Any, *, threshold: float = 0.72) -> bool:
    return output_similarity_score(left, right) >= threshold
...
def summarize_tool_lifecycle(tool_calls: list[dict[str, Any]] | None) -> list[dict[str, Any]] | None:
    if not tool_calls:
        return None
    ...
    for item in tool_calls:
        ...
        tool_name = item.get("tool_name") or item.get("name") or function.get("name")
        ...
        success_raw = item.get("success") or item.get("tool_success")
        success = bool(success_raw) if isinstance(success_raw, bool) else status in {"success", "ok", "completed"}
