"""MCP interception persistence (Slice 1.5).

Two durable tables that harden the MCP ingress before receipts:
  ...
  * ``mcp_interception_events`` — an append-only audit row for every
    ...
    is the precursor the receipt slice joins against.
"""
...
class McpToolBinding(Base):
    ...
    is_regex: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("0"))
    ...
    protected: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("1"))
...
class McpInterceptionEvent(Base):
    ...
    protected: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("0"))
    ...
    forward_attempted: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("0"))
    forward_succeeded: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("0"))
