...
def test_backend_production_deploy_uses_scoped_config_and_strict_smoke() -> None:
    ...
    assert "upload_args=(zroky-backend --path-as-root)" in workflow
    assert workflow.index('deploy_and_wait api "${RAILWAY_API_SERVICE_ID}" repo_root') < workflow.index(
        'deploy_and_wait worker "${RAILWAY_WORKER_SERVICE_ID}" backend_as_root'
